...
def batch_all_triplet_loss_with_scores_and_ids(
    ...
    if ids is None:
        ...
        # (N,N,N) tensor that ensures that ids match (e.g. loss only on same patient, ...)
...
def batch_all_triplet_loss_WST(
    ...
    # (labels, embeddings, ids margin_scores) belong together and must have same len 
    # -> anchor and negative pairs 
    #
    # embeddings_self_transform -> positive pairs 
    pairwise_dist = pairwise_distances(embeddings, metric=metric, squared=squared)
    anchor_negative_dist = pairwise_dist.unsqueeze(1)

    pairwise_dist_self = pairwise_distances_two_tensors(embeddings, embeddings_self_transform, metric=metric, squared=squared)
    anchor_positive_dist = pairwise_dist_self.unsqueeze(2)

    # Id must be same (e.g. same Patient; same side, same ROI)
    if ids is None:
        delta_id = torch.ones(1, dtype=torch.bool)
    else:
        delta_id = get_instance_mask_2D(ids=ids, device=embeddings.device).unsqueeze(0)  # (1,N,N)
    ...
    margin = margin_offset
    if margin_scores is not None:
        s_a = margin_scores.unsqueeze(0).unsqueeze(2)  # -> (1, N, 1)
        s_n = margin_scores.unsqueeze(0).unsqueeze(0)  # -> (1, 1, N)
    ...
    # Put to zero the invalid triplets
    # (where label(a) != label(p) or label(n) == label(a) or a == p)
    mask_triplet = get_triplet_mask_WST(labels)
    mask = mask_triplet & delta_id
    triplet_loss = mask.float() * triplet_loss
