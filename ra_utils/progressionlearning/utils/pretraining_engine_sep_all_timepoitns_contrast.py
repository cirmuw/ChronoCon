        ...
        for i in range(len(timepoints)):
            for j in range(len(timepoints)):
                if i != j: 
                    ...
                    loss_sup_1 =  1.0 - nn.functional.cosine_similarity(a_pcr_1, p_pcr_1, dim=-1).mean()

                    # population level for pcr == 0 patients
                    p_pcr_0 = p[label==0.0]
                    a_pcr_0 = a[label==0.0]
                    perm_a_pcr_0 = torch.randperm(a_pcr_0.size(0))
                    a_pcr_0 = a_pcr_0[perm_a_pcr_0]

                    loss_sup_0 =  1.0 - nn.functional.cosine_similarity(a_pcr_0, p_pcr_0, dim=-1).mean()

                    loss_sup = (loss_sup_0 + loss_sup_1)/2
