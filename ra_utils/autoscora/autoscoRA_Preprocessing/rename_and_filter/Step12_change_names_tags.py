...
dicom_server = "/project/autoscora/autoscoRA_images/H_images_of_interest_2_renamed_dicoms"
...
# check that all images in "images_of_interest_2" dir have img_of_interest==2
images_of_interest_2_files = os.listdir(dicom_server)
...
# check that no other images have img_of_interest==2
...
wrong_manual_names = [i for i, x in enumerate(pat_df_manual["img_of_interest"])
                      if x == 2
...
# --> conclusion: overwrite the filename_manual column for all images with img_of_interest == 2
for i, x in enumerate(pat_df_manual["img_of_interest"]):
...
list_of_filenames = [x for i, x in enumerate(pat_df_manual["filename_manual"])
                     if pat_df_manual.loc[i, "img_of_interest"] == 2]
list_of_H_dp_LR_names = [str(pat_df_manual.loc[i, "pat_id"]) + "_" +
                         str(pat_df_manual.loc[i, "study_date"]) + "_" +
                         pat_df_manual.loc[i, "bodypart_manual"] + "_" +
                         pat_df_manual.loc[i, "laterality_manual"] + "_" +
...
only_unique_H_dp_LR_names = len(set([x for x in list_of_H_dp_LR_names if list_of_H_dp_LR_names.count(x) > 1])) == 0

print("only unique filenames? " + str(only_unique_names))
print("only unique H_dp_L/R files for each pat_id + study_date? " + str(only_unique_H_dp_LR_names))
...
# check that all img_of_interest_2 names are in dir and v.v.
...
old_filepaths = [dicom_server + os.sep + i + ".dcm" for i in old_names]
...
for i in range(0, len(old_filepaths)):
    ...
    dcm_file.save_as(filename=old_filepaths[i], write_like_original=True)
