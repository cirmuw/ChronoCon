...
pat_df_manual_version = "pat_df_manual_2019-04-14_15-17-58.csv"
...
##########################################################
# put true candidates into H_images_of_interest_1_dicoms #
##########################################################

true_candidates_idx = [i for i, x in enumerate(pat_df_manual["comment"]) if "true_candidate" in x]

true_candidate_paths = pat_df_manual.loc[true_candidates_idx, "pixel_action_filepath"]

# set img_of_interest = 1
pat_df_manual.loc[true_candidates_idx, "img_of_interest"] = 1
...
for i in duplicates_df.iterrows():
    if pat_df_manual.loc[i[0], "inappropriate_manual"] != "app":
        ...
        if pat_df_manual.loc[i[0], "img_of_interest"] != 1:
            excluded_inapp = excluded_inapp + [i[0]]
        else:
...
duplicate_dir = re.sub(os.path.split(dicom_server)[1] + "$", "H_dp_duplicate_1_dicoms", dicom_server)
