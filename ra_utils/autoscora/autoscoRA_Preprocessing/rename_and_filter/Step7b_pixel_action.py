...
# directory
anonym_prob_all_dir = dicom_server + os.sep + "anonym_prob_all_fixed_dicoms"
...
# indices
splitpoint_idx = [i for i, x in enumerate(pat_df_manual['filename_new_dupl'])
                  if pat_df_manual.loc[i, 'splitpoint_manual'] not in ["none", "skipped", "TBD"]
                  and pat_df_manual.loc[i, 'bodypart_manual'] == "H"
                  and pat_df_manual.loc[i, 'view_position_manual'] != "lat"
                  and pat_df_manual.loc[i, 'inappropriate_manual'] != "inapp"
                  and not bool(re.search("four_limbs", pat_df_manual.loc[i, 'comment']))]

# put splitpoints, indices, img in a dict
splitpoint_dict = {idx: {"img": pat_df_manual.loc[idx, 'sub_sub_dir'] +
                                os.sep +
                                pat_df_manual.loc[idx, 'filename_new_dupl'] + ".dcm",
                         "split_x": pat_df_manual.loc[idx, 'splitpoint_manual']}
                   for idx in splitpoint_idx}

# directory
splitpoint_fixed_dir = dicom_server + os.sep + "splitpoint_fixed_dicoms"
...
for idx, values in splitpoint_dict.items():
    ...
    for panel, panel_xray in split_images.items():
        ...
        new_row = pat_df_manual.loc[idx, :].copy()
        ...
        old_panel_comment = pat_df_manual.loc[idx, "comment"]
        ...
        # print(new_row)
        # append new_row to pat_df_manual
        new_row = new_row.rename(index=np.shape(pat_df_manual)[0])
        pat_df_manual.loc[np.shape(pat_df_manual)[0], :] = new_row
        print(pat_df_manual.loc[np.shape(pat_df_manual)[0]-1, :])
pat_df_manual = pat_df_manual.astype(pat_df_manual_original.dtypes)

# set pat_df manual comment to "split_done" for original image
old_original_comment = pat_df_manual.loc[splitpoint_idx, "comment"]
...
pat_df_manual.loc[splitpoint_idx, "comment"] = new_original_comment
pat_df_manual.loc[splitpoint_idx, "comment_status"] = "ComYes"


# ------ 4 limbs splitting ------ # copied from above and changed, check that changed everything to four_limbs
# indices
four_limbs_idx = [i for i, x in enumerate(pat_df_manual['filename_new_dupl'])
                  if pat_df_manual.loc[i, 'splitpoint_manual'] not in ["none", "skipped", "TBD"]
                  and pat_df_manual.loc[i, 'bodypart_manual'] == "H"
                  and pat_df_manual.loc[i, 'view_position_manual'] != "lat"
                  and pat_df_manual.loc[i, 'inappropriate_manual'] != "inapp"
                  and bool(re.search("four_limbs", pat_df_manual.loc[i, 'comment']))]

# put splitpoints, indices, img in a dict - DONE MANUALLY
four_limbs_dict = {idx: {"img": pat_df_manual.loc[idx, 'sub_sub_dir'] +
                                os.sep +
                                pat_df_manual.loc[idx, 'filename_new_dupl'] + ".dcm",
                         "split_x": 2088,
                         "split_y": 2472}
...
# set pat_df manual comment to "split_done" for original image
old_original_comment = pat_df_manual.loc[four_limbs_idx, "comment"]
new_original_comment = [x if "split_done" in x
                        else "split_done" if x in ["none", "TBD"] else x + "__split_done"
                        for x in old_original_comment]
pat_df_manual.loc[four_limbs_idx, "comment"] = new_original_comment
pat_df_manual.loc[four_limbs_idx, "comment_status"] = "ComYes"
...
pat_df_manual["pixel_action_filepath"] = pat_df_manual["sub_sub_dir"] + os.sep + \
                                         pat_df_manual["filename_new_dupl"] + ".dcm"
...
pat_df_manual.loc[rot_idx, "pixel_action_filepath"] = rotated_all_dir + os.sep + \
                                                      pat_df_manual.loc[rot_idx, "filename_new_dupl"] + ".dcm"

pat_df_manual.loc[anonym_prob_idx, "pixel_action_filepath"] = anonym_prob_all_dir + os.sep + \
                                                      pat_df_manual.loc[anonym_prob_idx, "filename_new_dupl"] + ".dcm"
