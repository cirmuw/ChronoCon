...
dicom_server = "/project/autoscora/autoscoRA_images/F_images_of_interest_2_renamed_dicoms"
old_server = "/project/autoscora/autoscoRA_images/F_images_of_interest_2_dicoms"
...
pat_df_manual_version = "pat_df_duplicates/pat_df_manual_2021-05-17_21-15-27_Step10_2021-05-20_12-00-38_img_of_interest_FEET_2_2021-05-20_16-15-07_CORRECTED.csv"
...
#####################################################
# check that manual category-based names are unique #
#####################################################

# find all images with img_of_interest_FEET == 2
img_of_interest_2_idx = [i for i, x in enumerate(pat_df_manual["img_of_interest_FEET"])
                         if x == 2]

# check that all images in "images_of_interest_2" dir have img_of_interest_FEET==2
images_of_interest_2_files = [i for i in os.listdir(old_server) if not i.startswith(".")]
images_of_interest_2_names = [re.sub(".dcm", "", i) for i in images_of_interest_2_files]

all(pat_df_manual.loc[[x in images_of_interest_2_names for x in pat_df_manual["filename_new_dupl"]],
                      "img_of_interest_FEET"] == 2)

# check that no other images have img_of_interest_FEET==2
all(pat_df_manual.loc[[x not in images_of_interest_2_names for x in pat_df_manual["filename_new_dupl"]],
                      "img_of_interest_FEET"] != 2)

# check that they're all in img_of_interst_2 dir
all(i + ".dcm" in images_of_interest_2_files for i in
    pat_df_manual.loc[img_of_interest_2_idx, "filename_new_dupl"])

# check that filename_manual matches manual categories
wrong_manual_names = [i for i, x in enumerate(pat_df_manual["img_of_interest_FEET"])
                      if x == 2
                      and not bool(re.search("Com", pat_df_manual.loc[i, "filename_manual"]))
                      and not bool(re.search("UlnaBoth|PIPIVProx|PIPIIIProx", pat_df_manual.loc[i, "filename_manual"]))
                      and pat_df_manual.loc[i, "filename_manual"] !=
                      str(pat_df_manual.loc[i, "pat_id"]) + "_" +
                      str(pat_df_manual.loc[i, "study_date"]) + "_" +
                      pat_df_manual.loc[i, "bodypart_manual"] + "_" +
                      pat_df_manual.loc[i, "laterality_manual"] + "_" +
                      pat_df_manual.loc[i, "view_position_manual"] + "_" +
                      pat_df_manual.loc[i, "photometric_interpretation_new"] + "_" +
                      pat_df_manual.loc[i, "inverted_manual"] + "_" +
                      pat_df_manual.loc[i, "rotated_manual"] + "_" +
                      pat_df_manual.loc[i, "black_manual"] + "_" +
                      pat_df_manual.loc[i, "operated_manual"] + "_" +
                      pat_df_manual.loc[i, "inappropriate_manual"]
                      ]

# Divergences occur due to
# # "ComYes/No" only in filename_manual if looked at after adding comment to Fiji (ca. 300)
# # removed UlnaBoth|PIPIVProx|PIPIIIProx in operated_manual but not filename_manual (ca. 300)
# # RotYes in rotated_manual but not filename_manual (the remaining idx [3355, 22469, 23590])

# --> conclusion: overwrite the filename_manual column for all images with img_of_interest_FEET == 2
for i, x in enumerate(pat_df_manual["img_of_interest_FEET"]):
    if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2:
        if pat_df_manual.loc[i, "comment_status"] in ["TBD", ""]:
            comment_status_i = "ComTBD"
        else:
            comment_status_i = pat_df_manual.loc[i, "comment_status"]
        pat_df_manual.loc[i, "filename_manual"] = str(pat_df_manual.loc[i, "pat_id"]) + "_" + \
                                                  str(pat_df_manual.loc[i, "study_date"]) + "_" + \
                                                  pat_df_manual.loc[i, "bodypart_manual"] + "_" + \
                                                  pat_df_manual.loc[i, "laterality_manual"] + "_" + \
                                                  pat_df_manual.loc[i, "view_position_manual"] + "_" + \
                                                  ...
                                                  pat_df_manual.loc[i, "black_manual"] + "_" + \
                                                  pat_df_manual.loc[i, "operated_manual"] + "_" + \
                                                  pat_df_manual.loc[i, "inappropriate_manual"] + "_" + \
                                                  comment_status_i
        print(pat_df_manual.loc[i, "filename_manual"])

# Check uniqueness of manual names
list_of_filenames = [x for i, x in enumerate(pat_df_manual["filename_manual"])
                     if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2]
list_of_F_dp_LR_names = [str(pat_df_manual.loc[i, "pat_id"]) + "_" +
                         str(pat_df_manual.loc[i, "study_date"]) + "_" +
                         pat_df_manual.loc[i, "bodypart_manual"] + "_" +
                         pat_df_manual.loc[i, "laterality_manual"] + "_" +
                         pat_df_manual.loc[i, "view_position_manual"]
                         for i, x in enumerate(pat_df_manual["filename_manual"])
                         if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2]
only_unique_names = len(set([x for x in list_of_filenames if list_of_filenames.count(x) > 1])) == 0
only_unique_F_dp_LR_names = len(set([x for x in list_of_F_dp_LR_names if list_of_F_dp_LR_names.count(x) > 1])) == 0

print("only unique filenames? " + str(only_unique_names))
print("only unique F_dp_L/R files for each pat_id + study_date? " + str(only_unique_F_dp_LR_names))

# save pat_df_manual with changed filename_manual
# current_datetime = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
# new_pat_df_filename = output_folder + os.sep + os.path.splitext(pat_df_manual_version)[0] + \
#                       "_NameChange_" + current_datetime + ".csv"
# pat_df_manual.to_csv(new_pat_df_filename)


################################################
# rename files + change "IssuerofPatientID tag #
################################################

# check that all img_of_interest_FEET_2 names are in dir and v.v.
old_names = [x for i, x in enumerate(pat_df_manual["filename_new_dupl"])
             if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2]
new_names = [x for i, x in enumerate(pat_df_manual["filename_manual"])
             if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2]
print("pat_df_manual filenames == listdir() filenames?  "
      + str(set(old_names) == set(images_of_interest_2_names)) + " "
      + str(all([x in old_names for x in images_of_interest_2_names])) + " "
      + str(all([x in images_of_interest_2_names for x in old_names]))
      )

old_filepaths = [old_server + os.sep + i + ".dcm" for i in old_names]
new_filepaths = [dicom_server + os.sep + i + ".dcm" for i in new_names]
new_bodypart = [x for i, x in enumerate(pat_df_manual["bodypart_manual"])
                if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2]
new_laterality = [x for i, x in enumerate(pat_df_manual["laterality_manual"])
                  if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2]
new_view_position = [x for i, x in enumerate(pat_df_manual["view_position_manual"])
                     if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2]

# check that only "DEFAULT" in IssuerofPatientID tag
not_DEFAULT = []
no_issuer_of_patient_id = []
for file_i in old_filepaths:
    xray = XRay(file_i)
    if "(0010, 0021)" in [str(i) for i in xray.img.keys()]:
        if xray.img.IssuerOfPatientID != "DEFAULT":
            not_DEFAULT = not_DEFAULT + [file_i]
    else:
        no_issuer_of_patient_id = no_issuer_of_patient_id + [file_i]
    del xray
    gc.collect()
print("IssuerOfPatientID == 'DEFAULT' for all images?" + str(len(not_DEFAULT) == 0))

# change IssuerOfPatientID tag and filepath name
new_tag_list = []
for i in range(0, len(old_filepaths)):
    dcm_file = pydicom.dcmread(old_filepaths[i])
    # put filename into IssuerOfPatientID
    if (0x010, 0x021) in dcm_file:
        dcm_file.IssuerOfPatientID = new_names[i]
    else:
        new_tag_list = new_tag_list + [new_names[i]]
        dcm_file.add_new((0x010, 0x021), 'LO', new_names[i])
    # change manual category tags
    dcm_file.BodyPartExamined = new_bodypart[i]
    dcm_file.Laterality = new_laterality[i]
    dcm_file.ViewPosition = new_view_position[i]
    # save header-edited dicom file at new location
    dcm_file.save_as(filename=new_filepaths[i], write_like_original=True)
    # os.rename(old_filepaths[i], new_filepaths[i])
    del dcm_file
    gc.collect()


print("files without tag IssuerOfPatientID:")
print(new_tag_list)
