...
# select feet of int #
#######################
foot_of_interest_feet = list([pat_df_manual.loc[i, "filename_manual"]
                              for i, x in enumerate(pat_df_manual["filename_manual"])
                              if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2])

foot_of_interest_feet_R = list([pat_df_manual.loc[i, "filename_manual"]
                                for i, x in enumerate(pat_df_manual["filename_manual"])
                                if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2
                                and pat_df_manual.loc[i, "laterality_manual"] == "R"])

foot_of_interest_feet_L = list([pat_df_manual.loc[i, "filename_manual"]
                                for i, x in enumerate(pat_df_manual["filename_manual"])
                                if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2
                                and pat_df_manual.loc[i, "laterality_manual"] == "L"])

foot_of_interest_visits = list(set([str(pat_df_manual.loc[i, "pat_id"]) + "_" + str(pat_df_manual.loc[i, "study_date"])
                                    for i, x in enumerate(pat_df_manual["filename_manual"])
                                    if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2]))

foot_of_interest_visits_R = list([str(pat_df_manual.loc[i, "pat_id"]) + "_" + str(pat_df_manual.loc[i, "study_date"])
                                  for i, x in enumerate(pat_df_manual["filename_manual"])
                                  if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2
                                  and pat_df_manual.loc[i, "laterality_manual"] == "R"])

foot_of_interest_visits_L = list([str(pat_df_manual.loc[i, "pat_id"]) + "_" + str(pat_df_manual.loc[i, "study_date"])
                                  for i, x in enumerate(pat_df_manual["filename_manual"])
                                  if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2
                                  and pat_df_manual.loc[i, "laterality_manual"] == "L"])

foot_of_interest_visits_medstream = [x for i, x in enumerate(pat_df_medstream["id_RO_nr"])
                                     if re.sub("-", "", x) in foot_of_interest_visits]

foot_of_interest_visits_R_medstream = [x for i, x in enumerate(pat_df_medstream["id_RO_nr"])
                                       if re.sub("-", "", x) in foot_of_interest_visits_R]

foot_of_interest_visits_L_medstream = [x for i, x in enumerate(pat_df_medstream["id_RO_nr"])
                                       if re.sub("-", "", x) in foot_of_interest_visits_L]

foot_of_interest_patients = list(set([x for i, x in enumerate(pat_df_manual["pat_id"])
                                      if pat_df_manual.loc[i, "img_of_interest_FEET"] == 2]))
foot_of_interest_patients_medstream = list(set([x for i, x in enumerate(pat_df_medstream["id_nr"])
                                                if x in foot_of_interest_patients]))


################################################################################################
# pat-level: prepare medstream for visits corresponding to at least one foot img_of_interest_FEET_2 #
################################################################################################

# prepare R and L medstream visits
foot_of_interest_visits_medstream_idx = [i for i, x in enumerate(pat_df_medstream["id_RO_nr"])
                                         if x in foot_of_interest_visits_medstream]

foot_of_interest_visits_medstream_df = deepcopy(pat_df_medstream.iloc[foot_of_interest_visits_medstream_idx, :])

# remove unneeded columns
short_medstream = foot_of_interest_visits_medstream_df.drop(columns=removable_joints)
...
# prepare R and L medstream visits
foot_of_interest_visits_medstream_R_idx = [i for i, x in enumerate(pat_df_medstream["id_RO_nr"])
                                           if x in foot_of_interest_visits_R_medstream]

foot_of_interest_visits_medstream_R_df = deepcopy(pat_df_medstream.iloc[foot_of_interest_visits_medstream_R_idx, :])

foot_of_interest_visits_medstream_L_idx = [i for i, x in enumerate(pat_df_medstream["id_RO_nr"])
                                           if x in foot_of_interest_visits_L_medstream]

foot_of_interest_visits_medstream_L_df = deepcopy(pat_df_medstream.iloc[foot_of_interest_visits_medstream_L_idx, :])

# remove unneeded columns
short_medstream_R = foot_of_interest_visits_medstream_R_df.drop(columns=removable_joints + left_foot_joints)
short_medstream_L = foot_of_interest_visits_medstream_L_df.drop(columns=removable_joints + right_foot_joints)

# rename joints columns
col_rename_dict_R = dict(zip(right_foot_joints, neutral_foot_joints))
short_medstream_R.rename(columns=col_rename_dict_R, inplace=True)

col_rename_dict_L = dict(zip(left_foot_joints, neutral_foot_joints))
short_medstream_L.rename(columns=col_rename_dict_L, inplace=True)

# NOTE: since each manual id_date_F_laterality_dp is unique in pat_df_manual, I can use this as identifier

# prepare new pat_df_manual version
pat_df_manual_id_added = deepcopy(pat_df_manual)

pat_df_manual_id_added["img_id"] = None
for i, x in enumerate(pat_df_manual_id_added["img_of_interest_FEET"]):
    if x == 2:
        pat_df_manual_id_added.loc[i, "img_id"] = \
            re.sub("(_MOne|_MTwo).*$", "", pat_df_manual_id_added.loc[i, "filename_manual"])

pat_df_manual_id_added_2_idx = [idx for idx, x in enumerate(pat_df_manual_id_added["img_of_interest_FEET"])
                                if x == 2]
...
short_medstream_R["img_id"] = [re.sub("-", "", x) + "_" + "F_R_dp"
                               for i, x in enumerate(short_medstream_R["id_RO_nr"])]

short_medstream_L["img_id"] = [re.sub("-", "", x) + "_" + "F_L_dp"
...
#                                       "pat_df_img_interest_2_renamed_mirrored_inverted_FEET" + \
#                                       os.sep + "pat_df_medstream_manual_F_dp_img_of_int_2_" + \
...
# add columns r_total_ero, r_total_JSN, r_total_score, etc.
short_medstream_R_L_manual['r_total_one_foot'] = \
    short_medstream_R_L_manual.loc[:, neutral_foot_joints].sum(axis=1, skipna=False)
short_medstream_R_L_manual['r_total_one_foot_JSN'] = \
    short_medstream_R_L_manual.loc[:, neutral_foot_JSN].sum(axis=1, skipna=False)
short_medstream_R_L_manual['r_total_one_foot_ero'] = \
    short_medstream_R_L_manual.loc[:, neutral_foot_ero].sum(axis=1, skipna=False)

print("any NaNs in r_total_one_foot: " + str(any(pd.isnull(short_medstream_R_L_manual['r_total_one_foot']))))
print("any NaNs in r_total_one_foot_JSN: " + str(any(pd.isnull(short_medstream_R_L_manual['r_total_one_foot_JSN']))))
print("any NaNs in r_total_one_foot_ero: " + str(any(pd.isnull(short_medstream_R_L_manual['r_total_one_foot_ero']))))
...
total_ax.hist(short_medstream_R_L_manual.loc[short_medstream_R_L_manual.set == 'train', 'r_total_one_foot'],
              bins=nbin, label='train',
              density=normed_flag, cumulative=cumulative_flag, histtype=histtype)
total_ax.hist(short_medstream_R_L_manual.loc[short_medstream_R_L_manual.set == 'val', 'r_total_one_foot'],
              bins=nbin, alpha=0.7, label='val',
              density=normed_flag, cumulative=cumulative_flag, histtype=histtype)
total_ax.hist(short_medstream_R_L_manual.loc[short_medstream_R_L_manual.set == 'test', 'r_total_one_foot'],
...
jsn_ax.hist(short_medstream_R_L_manual.loc[short_medstream_R_L_manual.set == 'train', 'r_total_one_foot_JSN'],
            bins=nbin, label='train',
            density=True, cumulative=cumulative_flag, histtype=histtype)
jsn_ax.hist(short_medstream_R_L_manual.loc[short_medstream_R_L_manual.set == 'val', 'r_total_one_foot_JSN'],
            bins=nbin, alpha=0.7, label='val',
            density=normed_flag, cumulative=cumulative_flag, histtype=histtype)
jsn_ax.hist(short_medstream_R_L_manual.loc[short_medstream_R_L_manual.set == 'test', 'r_total_one_foot_JSN'],
            bins=nbin, alpha=0.5, label='test',
            density=normed_flag, cumulative=cumulative_flag, histtype=histtype)
...
ero_ax.hist(short_medstream_R_L_manual.loc[short_medstream_R_L_manual.set == 'train', 'r_total_one_foot_ero'],
            bins=nbin, label='train',
            density=normed_flag, cumulative=cumulative_flag, histtype=histtype)
ero_ax.hist(short_medstream_R_L_manual.loc[short_medstream_R_L_manual.set == 'val', 'r_total_one_foot_ero'],
            bins=nbin, alpha=0.7, label='val',
            density=normed_flag, cumulative=cumulative_flag, histtype=histtype)
ero_ax.hist(short_medstream_R_L_manual.loc[short_medstream_R_L_manual.set == 'test', 'r_total_one_foot_ero'],
            bins=nbin, alpha=0.5, label='test',
            density=normed_flag, cumulative=cumulative_flag, histtype=histtype)
...
short_medstream_filename = output_folder + os.sep + 'data_split' + os.sep + \
                           "pat_df_medstream_F_dp_img_of_int_2_summary_cols_pat_sets_strat" + \
...
short_medstream_R_L_manual_filename = output_folder + os.sep + 'data_split' + os.sep + \
                                      "pat_df_medstream_manual_F_dp_img_of_int_2_summary_cols_pat_sets_strat" + \
                                      str(strat_seed) + "_split" + str(split_seed) + "_" + \
                                      current_datetime + ".csv"
short_medstream_R_L_manual.to_csv(short_medstream_R_L_manual_filename)
