...
def main():
    ...
    with mlflow.start_run(experiment_id=experiment_id, run_name=config["run_name"], nested=True):
        ...
        def train_loop(
            ...
            for epoch in range(epochs):
                ...
                if val_mpe < best_val_mpe:
                    ...
                    best_model_wts = copy.deepcopy(model.state_dict())
                    best_heatmap_wts = copy.deepcopy(
                        heatmap_generator.state_dict())
                    epochs_no_improve = 0

                    # Log current model as the best so far
                    mlflow.pytorch.log_model(model, artifact_path="best_model")
                    mlflow.pytorch.log_model(
                        heatmap_generator, "best_heatmap_generator")
                else:
                    epochs_no_improve += 1
        ...
        mlflow.pytorch.log_model(
            heatmap_generator, "heatmap_generator_checkpoint")
